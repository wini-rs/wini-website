...
pub fn add_cache(mut response: Response, cache_rule: &str) -> Response {
    response
        .headers_mut()
        .insert(CACHE_CONTROL, HeaderValue::from_str(cache_rule).unwrap());
...
pub async fn html_middleware(
    ...
    res_parts.headers.insert(
        CACHE_CONTROL,
        HeaderValue::from_str(&SERVER_CONFIG.cache.get(CacheCategory::Html)).unwrap(),
    );
